#!/usr/bin/env python3
"""Re-process Dec & Jan MIS with freight from CSV exports, then push to Google Sheets."""
...
# ── Same configs as dec_mis_generator.py ────────────────────────────────────
...
def extract_freight_from_csv(csv_file):
    """Extract freight from Shiprocket CSV export, deduplicated by Order ID."""
    freight = {}
    with open(csv_file, 'r') as f:
        ...
        for row in reader:
            ...
            if oid and val and oid not in freight:
                try:
                    freight[oid] = float(val)
                except ValueError:
                    pass
    return freight
...
# ── Google Sheets push (categorized) ────────────────────────────────────────
...
def main():
    print("=" * 60)
    print("  REFRESHING FREIGHT FOR DECEMBER & JANUARY MIS")
    print("=" * 60)

    # Extract freight from CSVs
    print("\nLoading freight from CSV exports...")
    dec_freight = extract_freight_from_csv(f"{BASE_DIR}/Decdata.csv")
    jan_freight = extract_freight_from_csv(
        f"{BASE_DIR}/secure_458644_reports_1773166327582881847-0c4f90e07051f9ee6365c62e7e17b121-.csv")
    print(f"  Dec freight: {len(dec_freight)} orders ({sum(1 for v in dec_freight.values() if v > 0)} with charges)")
    print(f"  Jan freight: {len(jan_freight)} orders ({sum(1 for v in jan_freight.values() if v > 0)} with charges)")

    # Load cached raw orders
    with open(f"{BASE_DIR}/dec_orders_raw.json") as f:
        dec_orders = json.load(f)
    with open(f"{BASE_DIR}/jan_orders_raw.json") as f:
        jan_orders = json.load(f)

    # Re-process with freight
    print("\nProcessing December...")
    dec_data = process_orders(dec_orders, dec_freight)
    print("\nProcessing January...")
    jan_data = process_orders(jan_orders, jan_freight)

    # Save updated MIS data
    with open(f"{BASE_DIR}/dec_mis_data.json", "w") as f:
        json.dump(dec_data, f, indent=2)
    with open(f"{BASE_DIR}/jan_mis_data.json", "w") as f:
        json.dump(jan_data, f, indent=2)
    print("\nSaved updated dec_mis_data.json and jan_mis_data.json")
    ...
    push_to_gsheet(sh, "December 2025 MIS", dec_data)
    time.sleep(2)
    push_to_gsheet(sh, "January 2026 MIS", jan_data)

    print(f"\n{'='*60}")
    print(f"  All done! {SHEET_URL}")
    print(f"{'='*60}")
