#!/usr/bin/env python3
"""Fetch Oct & Nov 2025 orders from Shiprocket API, process, and push to Google Sheets."""
...
def fetch_orders(date_from, date_to, month_label):
    all_orders = []
    page = 1
    per_page = 200
    print(f"\nFetching {month_label} orders ({date_from} to {date_to})...")
    ...
    while True:
        ...
        total_pages = meta.get("pagination", {}).get("total_pages", 0)
...
def process_orders(orders, freight_map=None):
    if freight_map is None:
        freight_map = {}
...
def main():
    print("=" * 60)
    print("  OCTOBER & NOVEMBER 2025 MIS GENERATOR")
    print("=" * 60)

    months = [
        ("October 2025", "2025-10-01", "2025-10-31", "oct"),
        ("November 2025", "2025-11-01", "2025-11-30", "nov"),
    ]

    all_month_data = {}

    for month_label, date_from, date_to, prefix in months:
        raw_path = os.path.join(BASE_DIR, f"{prefix}_orders_raw.json")

        # Fetch or load cache
        if os.path.exists(raw_path):
            print(f"\nLoading cached {month_label} orders...")
            with open(raw_path) as f:
                orders = json.load(f)
            print(f"  Loaded {len(orders)} orders from cache")
        else:
            orders = fetch_orders(date_from, date_to, month_label)
            if orders is None:
                print(f"Failed to fetch {month_label}. Skipping.")
                continue
            with open(raw_path, "w") as f:
                json.dump(orders, f, default=str)
            print(f"  Saved to {raw_path}")

        # Process (no freight for now — user can add CSV later)
        print(f"\nProcessing {month_label}...")
        product_data = process_orders(orders)

        # Save MIS data
        mis_path = os.path.join(BASE_DIR, f"{prefix}_mis_data.json")
        with open(mis_path, "w") as f:
            json.dump(product_data, f, indent=2)
        print(f"  Saved to {mis_path}")

        all_month_data[month_label] = product_data
    ...
    for month_label, _, _, _ in months:
        if month_label in all_month_data:
            push_to_gsheet(sh, f"{month_label} MIS", all_month_data[month_label])
            time.sleep(2)

    # Summary
    print(f"\n{'='*60}")
    for month_label in all_month_data:
        pd = all_month_data[month_label]
        total_ord = sum(d["total_orders"] for d in pd.values())
        total_del = sum(d["delivered"] for d in pd.values())
        total_rev = sum(d["revenue"] for d in pd.values())
        total_rto = sum(d["rto"] for d in pd.values())
        total_shipped = sum(d["shipped"] for d in pd.values())
        print(f"\n  {month_label}:")
        print(f"    Orders: {total_ord:,} | Delivered: {total_del:,} | Revenue: ₹{total_rev:,.0f}")
        if total_shipped > 0:
            print(f"    RTO%: {total_rto/total_shipped*100:.1f}% | Delivered%: {total_del/total_ord*100:.1f}%")
    print(f"\n{'='*60}")
